...
from products.replay_vision.backend.temporal.lenses import lens_from_observation
...
async def call_lens_provider_activity(inputs: CallLensProviderInputs) -> LensCallOutput:
    ...
    lens = await sync_to_async(_load_lens_from_snapshot)(inputs.observation_id, inputs.team_id)
...
def _load_lens_from_snapshot(observation_id: UUID, team_id: int) -> BaseLens:
    """Hydrate the lens from `observation.lens_config_snapshot`, not the live row."""
    observation = ReplayObservation.objects.select_related("lens").filter(pk=observation_id, team_id=team_id).first()
    if observation is None:
        raise ApplicationError(f"ReplayObservation {observation_id} not found for team {team_id}", non_retryable=True)
    return lens_from_observation(observation)
...
async def _call_with_retry(*, lens: BaseLens, model: str, prompt_parts: list[types.Part], team_id: int) -> BaseModel:
    ...
    # The activity owns its own retry exhaustion via the loop above; the workflow's `_PROVIDER_CALL_RETRY`
    # will not retry on this `ApplicationError(non_retryable=True)`.
