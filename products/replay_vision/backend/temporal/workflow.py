...
# Upload: deterministic failures (asset missing, polling timeout) shouldn't burn retries on a re-upload
# that just leaks another Gemini file before the cleanup sweep reaps it.
...
# Provider call owns its own retry exhaustion via an in-body re-prompt loop; the workflow shouldn't
# additionally retry — `ApplicationError(non_retryable=True)` from the activity already prevents that,
# but a single attempt makes the layering explicit.
_PROVIDER_CALL_RETRY = common.RetryPolicy(maximum_attempts=1)
