...
class TestMonitorLens:
    ...
    def test_build_prompt_includes_team_name_user_intent_and_task(self) -> None:
        ...
        rendered = lens.build_prompt(
            team_name="Acme", columns=["event", "$current_url"], events=[["$pageview", "/cart"]]
        )
    ...
    def test_build_prompt_with_no_events_renders_explicit_marker(self) -> None:
        lens = lens_from_db(_build_replay_lens())
        rendered = lens.build_prompt(team_name="Acme", columns=[], events=[])
        assert "(no events captured during the session)" in rendered

    def test_emits_signals_does_not_inject_side_mission_yet(self) -> None:
        # Phase 4 will populate the side mission; until then, emits_signals=True is a no-op on the prompt.
        lens = lens_from_db(_build_replay_lens(emits_signals=True))
        rendered_on = lens.build_prompt(team_name="Acme", columns=[], events=[])
        rendered_off = lens_from_db(_build_replay_lens(emits_signals=False)).build_prompt(
            team_name="Acme", columns=[], events=[]
        )
